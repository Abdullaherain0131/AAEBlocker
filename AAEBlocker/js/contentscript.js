...
vAPI.hideStyle = 'display:none!important center/contain no-repeat #0b0f19!important;opacity:0.8!important;';
    ...
    const getCollapseToken = ( ) => {
        if ( collapseToken === undefined ) {
            ...
            vAPI.userStylesheet.add(
                `[${collapseToken}]\n{display:none!important center/contain no-repeat #0b0f19!important;opacity:0.8!important;}`,
                true
            );
        }
        ...
        const censorWords = () => {
            ...
            els.forEach(el => {
                ...
                if (words.some(w => txt.includes(w))) {
                    ...
                    console.log("[AAEBlocker] Censored element for word match");
                }
            });
        };
        censorWords();
...
// AAEBlocker Ad Replacer CSS
const logoUrlAAE = chrome.runtime.getURL('img/logo.png');
const styleAAE = document.createElement('style');
styleAAE.textContent = `
ins.adsbygoogle, iframe[id^=\"google_ads_iframe\"], iframe[src*=\"doubleclick\"], iframe[src*=\"amazon-adsystem\"], .ad-container, .ad-slot, .ad-wrapper, .banner-ad, [id*=\"banner-ad\"] {
  background-image: url('${logoUrlAAE}') !important;
...
document.head.appendChild(styleAAE);
